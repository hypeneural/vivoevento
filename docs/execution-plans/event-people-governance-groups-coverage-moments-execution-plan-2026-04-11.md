...
- `Whatâ€™s New in WCAG 2.2`: https://www.w3.org/WAI/standards-guidelines/wcag/new-in-22/
...
- [x] criar `event_relational_collections`
- [x] criar `event_relational_collection_items`
- [x] criar recipe engine de colecoes relacionais
- [x] criar entregas prontas por vinculo
- [x] preparar trilha guest-facing derivada
...
- colecoes relacionais locais agora existem com:
  - `GET /people/relational-collections`
  - `POST /people/relational-collections/refresh`
  - `event_relational_collections`
  - `event_relational_collection_items`
  - `person_best_of`, `pair_best_of`, `group_best_of`, `family_moment` e `must_have_delivery`
  - painel `Momentos e entregas` dentro de `EventPeoplePage`
- a trilha guest-facing derivada agora existe com:
  - `GET /api/v1/public/people-collections/{token}`
  - rota publica `/momentos/:token`
  - `public_url` e `public_api_url` expostos nas colecoes `public_ready`
  - pagina publica dedicada carregando apenas itens publicados da colecao tokenizada
