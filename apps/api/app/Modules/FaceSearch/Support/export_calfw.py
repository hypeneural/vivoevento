...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export CALFW into a reusable FaceSearch holdout manifest.")
...
def group_people(root: Path, min_images_per_person: int) -> list[dict[str, Any]]:
    grouped: dict[str, list[Path]] = {}

    for image_path in sorted(root.glob("*.jpg"), key=lambda item: item.name.lower()):
        stem = image_path.stem

        if "_" not in stem:
            continue

        person_id, image_index = stem.rsplit("_", 1)

        if not image_index.isdigit():
            continue

        grouped.setdefault(person_id, []).append(image_path)

    people: list[dict[str, Any]] = []

    for person_id, images in grouped.items():
...
def export_entries(selected_people: list[dict[str, Any]], image_selection: str, output_dir: Path) -> tuple[list[dict[str, Any]], dict[str, list[str]]]:
    ...
    for person in selected_people:
        ...
        for index, image_path in enumerate(images, start=1):
            ...
            entry_id = f"calfw-{person_slug}-{index:02d}"
            ids_by_person[person_id].append(entry_id)

            entries.append({
                "id": entry_id,
                "dataset": "calfw",
                "event_id": "local-calfw",
                "person_id": person_id,
                "relative_path": relative_path.as_posix(),
                "scene_type": "single_prominent",
                "quality_label": "age_holdout",
                ...
                "notes": "Exported from local CALFW for age holdout calibration.",
            })
...
def main() -> int:
    ...
    if not root.is_dir():
        raise FileNotFoundError(f"CALFW root not found: {root}")

    people = sort_people(group_people(root, min_images_per_person), selection)
    ...
    if len(selected_people) < 2:
        raise RuntimeError("CALFW export requires at least two identities after filtering.")

    entries, ids_by_person = export_entries(selected_people, image_selection, output_dir)

    if len(ids_by_person) < 2 or len(entries) < 4:
        raise RuntimeError("CALFW export did not produce enough entries for holdout evaluation.")
    ...
    manifest = {
        ...
        "entries": entries,
        "notes": [
            "This manifest is intended for age holdout evaluation, not customer search.",
