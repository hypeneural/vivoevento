...
class RunManifestFaceSizeThresholdSweepCommand extends Command
{
    protected $signature = 'face-search:sweep-manifest-min-face-size
    ...
    protected $description = 'Run a real min_face_size_px sweep against a dataset manifest exported for FaceSearch calibration.';

    public function handle(ManifestFaceSizeThresholdSweepService $service): int
    {
        try {
            $report = $service->run(
    ...
    {
        $directory = (string) ($this->option('report-dir') ?: storage_path('app/face-search-manifest-min-face-size-sweep'));

        File::ensureDirectoryExists($directory);

        $filename = sprintf('%s-face-search-manifest-min-face-size-sweep.json', now()->format('Ymd-His-u'));
