...
class LfwLocalLoaderService
{
    ...
    public function run(
        ...
        $scriptPath = base_path('app/Modules/FaceSearch/Support/export_lfw.py');

        if (! is_dir($resolvedRoot)) {
            throw new RuntimeException(sprintf('LFW root [%s] does not exist.', $resolvedRoot));
        }

        if (! File::exists($scriptPath)) {
            throw new RuntimeException(sprintf('LFW exporter script [%s] does not exist.', $scriptPath));
        }
        ...
        if (! $result->successful()) {
            throw new RuntimeException(sprintf(
                'LFW loader failed with exit code [%s]. %s',
        ...
        try {
            ...
            throw new RuntimeException('LFW loader returned invalid JSON.', previous: $exception);
        }

        if (! is_array($payload) || ! is_string($payload['manifest_path'] ?? null)) {
            throw new RuntimeException('LFW loader did not return a valid manifest payload.');
        }

        if (! File::exists((string) $payload['manifest_path'])) {
            throw new RuntimeException(sprintf(
                'LFW loader reported manifest [%s], but the file does not exist.',
    ...
    private function resolveRoot(string $root): string
    {
        ...
        $candidates = [
            '%USERPROFILE%/Desktop/model/extracted/lfw/lfw',
            '%USERPROFILE%/Desktop/model/extracted/lfw/lfw-deepfunneled/lfw-deepfunneled',
        ];
        ...
        return $this->resolvePath($candidates[0]);
    }
    ...
    private function resolveOutputDirectory(string $outputDirectory): string
    {
        ...
        return storage_path(sprintf(
            'app/face-search-datasets/lfw/%s-lfw',
            now()->format('Ymd-His'),
        ));
    }
