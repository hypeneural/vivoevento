<?php

use Illuminate\Support\Str;

return [

    /*
    |--------------------------------------------------------------------------
    | Horizon Name
    |--------------------------------------------------------------------------
    |
    | This name appears in notifications and in the Horizon UI. Unique names
    | can be useful while running multiple instances of Horizon within an
    | application, allowing you to identify the Horizon you're viewing.
    |
    */

    'name' => env('HORIZON_NAME'),

    /*
    |--------------------------------------------------------------------------
    | Horizon Domain
    |--------------------------------------------------------------------------
    |
    | This is the subdomain where Horizon will be accessible from. If this
    | setting is null, Horizon will reside under the same domain as the
    | application. Otherwise, this value will serve as the subdomain.
    |
    */

    'domain' => env('HORIZON_DOMAIN'),

    /*
    |--------------------------------------------------------------------------
    | Horizon Path
    |--------------------------------------------------------------------------
    |
    | This is the URI path where Horizon will be accessible from. Feel free
    | to change this path to anything you like. Note that the URI will not
    | affect the paths of its internal API that aren't exposed to users.
    |
    */

    'path' => env('HORIZON_PATH', 'horizon'),

    /*
    |--------------------------------------------------------------------------
    | Horizon Redis Connection
    |--------------------------------------------------------------------------
    |
    | This is the name of the Redis connection where Horizon will store the
    | meta information required for it to function. It includes the list
    | of supervisors, failed jobs, job metrics, and other information.
    |
    */

    'use' => 'default',

    /*
    |--------------------------------------------------------------------------
    | Horizon Redis Prefix
    |--------------------------------------------------------------------------
    |
    | This prefix will be used when storing all Horizon data in Redis. You
    | may modify the prefix when you are running multiple installations
    | of Horizon on the same server so that they don't have problems.
    |
    */

    'prefix' => env(
        'HORIZON_PREFIX',
        Str::slug(env('APP_NAME', 'laravel'), '_').'_horizon:'
    ),

    /*
    |--------------------------------------------------------------------------
    | Horizon Route Middleware
    |--------------------------------------------------------------------------
    |
    | These middleware will get attached onto each Horizon route, giving you
    | the chance to add your own middleware to this list or change any of
    | the existing middleware. Or, you can simply stick with this list.
    |
    */

    'middleware' => ['web'],

    /*
    |--------------------------------------------------------------------------
    | Queue Wait Time Thresholds
    |--------------------------------------------------------------------------
    |
    | This option allows you to configure when the LongWaitDetected event
    | will be fired. Every connection / queue combination may have its
    | own, unique threshold (in seconds) before this event is fired.
    |
    */

    'waits' => [
        'redis:broadcasts' => (int) env('HORIZON_WAIT_BROADCASTS', 10),
        'redis:face-index' => (int) env('HORIZON_WAIT_FACE_INDEX', 180),
        'redis:media-variants' => (int) env('HORIZON_WAIT_MEDIA_VARIANTS', 45),
        'redis:media-audit' => (int) env('HORIZON_WAIT_MEDIA_AUDIT', 45),
        'redis:media-process' => (int) env('HORIZON_WAIT_MEDIA_PROCESS', 90),
        'redis:media-safety' => (int) env('HORIZON_WAIT_MEDIA_SAFETY', 60),
        'redis:media-vlm' => (int) env('HORIZON_WAIT_MEDIA_VLM', 90),
        'redis:media-publish' => (int) env('HORIZON_WAIT_MEDIA_PUBLISH', 30),
        'redis:whatsapp-inbound' => (int) env('HORIZON_WAIT_WHATSAPP_INBOUND', 30),
        'redis:whatsapp-send' => (int) env('HORIZON_WAIT_WHATSAPP_SEND', 60),
        'redis:whatsapp-sync' => (int) env('HORIZON_WAIT_WHATSAPP_SYNC', 120),
        'redis:telegram-send' => (int) env('HORIZON_WAIT_TELEGRAM_SEND', 60),
        'redis:default' => 60,
    ],

    /*
    |--------------------------------------------------------------------------
    | Job Trimming Times
    |--------------------------------------------------------------------------
    |
    | Here you can configure for how long (in minutes) you desire Horizon to
    | persist the recent and failed jobs. Typically, recent jobs are kept
    | for one hour while all failed jobs are stored for an entire week.
    |
    */

    'trim' => [
        'recent' => 60,
        'pending' => 60,
        'completed' => 60,
        'recent_failed' => 10080,
        'failed' => 10080,
        'monitored' => 10080,
    ],

    /*
    |--------------------------------------------------------------------------
    | Silenced Jobs
    |--------------------------------------------------------------------------
    |
    | Silencing a job will instruct Horizon to not place the job in the list
    | of completed jobs within the Horizon dashboard. This setting may be
    | used to fully remove any noisy jobs from the completed jobs list.
    |
    */

    'silenced' => [
        // App\Jobs\ExampleJob::class,
    ],

    'silenced_tags' => [
        // 'notifications',
    ],

    /*
    |--------------------------------------------------------------------------
    | Metrics
    |--------------------------------------------------------------------------
    |
    | Here you can configure how many snapshots should be kept to display in
    | the metrics graph. This will get used in combination with Horizon's
    | `horizon:snapshot` schedule to define how long to retain metrics.
    |
    */

    'metrics' => [
        'trim_snapshots' => [
            'job' => 24,
            'queue' => 24,
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Fast Termination
    |--------------------------------------------------------------------------
    |
    | When this option is enabled, Horizon's "terminate" command will not
    | wait on all of the workers to terminate unless the --wait option
    | is provided. Fast termination can shorten deployment delay by
    | allowing a new instance of Horizon to start while the last
    | instance will continue to terminate each of its workers.
    |
    */

    'fast_termination' => true,

    /*
    |--------------------------------------------------------------------------
    | Memory Limit (MB)
    |--------------------------------------------------------------------------
    |
    | This value describes the maximum amount of memory the Horizon master
    | supervisor may consume before it is terminated and restarted. For
    | configuring these limits on your workers, see the next section.
    |
    */

    'memory_limit' => 256,

    /*
    |--------------------------------------------------------------------------
    | Queue Worker Configuration
    |--------------------------------------------------------------------------
    |
    | Here you may define the queue worker settings used by your application
    | in all environments. These supervisors and settings handle all your
    | queued jobs and will be provisioned by Horizon during deployment.
    |
    */

    'defaults' => [
        'supervisor-broadcasts' => [
            'connection' => 'redis',
            'queue' => ['broadcasts'],
            'balance' => false,
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 1500,
            'memory' => 192,
            'tries' => 3,
            'timeout' => 30,
            'nice' => 0,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-webhooks' => [
            'connection' => 'redis',
            'queue' => ['webhooks'],
            'balance' => false,
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 1500,
            'memory' => 192,
            'tries' => 5,
            'timeout' => 30,
            'nice' => 0,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-media-download' => [
            'connection' => 'redis',
            'queue' => ['media-download'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 750,
            'memory' => 256,
            'tries' => 3,
            'timeout' => 120,
            'nice' => 0,
            'backoff' => 10,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-media-variants' => [
            'connection' => 'redis',
            'queue' => ['media-variants'],
            'balance' => false,
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 1000,
            'memory' => 512,
            'tries' => 3,
            'timeout' => 360,
            'nice' => 0,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-media-audit' => [
            'connection' => 'redis',
            'queue' => ['media-audit'],
            'balance' => false,
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 1000,
            'memory' => 320,
            'tries' => 3,
            'timeout' => 60,
            'nice' => 0,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-media-process' => [
            'connection' => 'redis',
            'queue' => ['media-process'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1200,
            'maxJobs' => 300,
            'memory' => 512,
            'tries' => 2,
            'timeout' => 170,
            'nice' => 0,
            'backoff' => 15,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 5,
        ],
        'supervisor-face-index' => [
            'connection' => 'redis',
            'queue' => ['face-index'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1200,
            'maxJobs' => 250,
            'memory' => 512,
            'tries' => 2,
            'timeout' => 170,
            'nice' => 0,
            'backoff' => 20,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 5,
        ],
        'supervisor-media-safety' => [
            'connection' => 'redis',
            'queue' => ['media-safety'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 500,
            'memory' => 320,
            'tries' => 2,
            'timeout' => 90,
            'nice' => 0,
            'backoff' => 15,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-media-vlm' => [
            'connection' => 'redis',
            'queue' => ['media-vlm'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1200,
            'maxJobs' => 200,
            'memory' => 512,
            'tries' => 2,
            'timeout' => 120,
            'nice' => 0,
            'backoff' => 20,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 5,
        ],
        'supervisor-media-publish' => [
            'connection' => 'redis',
            'queue' => ['media-publish'],
            'balance' => false,
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 1200,
            'memory' => 192,
            'tries' => 3,
            'timeout' => 45,
            'nice' => 0,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-whatsapp-inbound' => [
            'connection' => 'redis',
            'queue' => ['whatsapp-inbound'],
            'balance' => false,
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 1500,
            'memory' => 192,
            'tries' => 3,
            'timeout' => 45,
            'nice' => 0,
            'backoff' => 5,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-whatsapp-send' => [
            'connection' => 'redis',
            'queue' => ['whatsapp-send'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 800,
            'memory' => 192,
            'tries' => 3,
            'timeout' => 90,
            'nice' => 0,
            'backoff' => 10,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-whatsapp-sync' => [
            'connection' => 'redis',
            'queue' => ['whatsapp-sync'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 500,
            'memory' => 192,
            'tries' => 3,
            'timeout' => 120,
            'nice' => 5,
            'backoff' => 15,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 5,
        ],
        'supervisor-telegram-send' => [
            'connection' => 'redis',
            'queue' => ['telegram-send'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 1800,
            'maxJobs' => 800,
            'memory' => 192,
            'tries' => 3,
            'timeout' => 90,
            'nice' => 0,
            'backoff' => 10,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-notifications' => [
            'connection' => 'redis',
            'queue' => ['notifications'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 3600,
            'maxJobs' => 1000,
            'memory' => 128,
            'tries' => 3,
            'timeout' => 60,
            'nice' => 0,
            'backoff' => 10,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
        'supervisor-default' => [
            'connection' => 'redis',
            'queue' => ['default', 'analytics', 'billing'],
            'balance' => 'auto',
            'autoScalingStrategy' => 'time',
            'maxProcesses' => 1,
            'maxTime' => 3600,
            'maxJobs' => 1000,
            'memory' => 128,
            'tries' => 3,
            'timeout' => 60,
            'nice' => 0,
            'backoff' => 10,
            'balanceMaxShift' => 1,
            'balanceCooldown' => 3,
        ],
    ],

    'environments' => [
        'production' => [
            'supervisor-broadcasts' => [
                'minProcesses' => (int) env('HORIZON_BROADCASTS_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_BROADCASTS_MAX_PROCESSES', 2),
            ],
            'supervisor-webhooks' => [
                'minProcesses' => (int) env('HORIZON_WEBHOOKS_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_WEBHOOKS_MAX_PROCESSES', 2),
            ],
            'supervisor-media-download' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_DOWNLOAD_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_DOWNLOAD_MAX_PROCESSES', 3),
            ],
            'supervisor-media-variants' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_VARIANTS_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_VARIANTS_MAX_PROCESSES', 2),
            ],
            'supervisor-media-audit' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_AUDIT_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_AUDIT_MAX_PROCESSES', 2),
            ],
            'supervisor-face-index' => [
                'minProcesses' => (int) env('HORIZON_FACE_INDEX_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_FACE_INDEX_MAX_PROCESSES', 2),
            ],
            'supervisor-media-process' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_PROCESS_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_PROCESS_MAX_PROCESSES', 3),
            ],
            'supervisor-media-safety' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_SAFETY_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_SAFETY_MAX_PROCESSES', 2),
            ],
            'supervisor-media-vlm' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_VLM_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_VLM_MAX_PROCESSES', 2),
            ],
            'supervisor-media-publish' => [
                'minProcesses' => (int) env('HORIZON_MEDIA_PUBLISH_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_MEDIA_PUBLISH_MAX_PROCESSES', 2),
            ],
            'supervisor-whatsapp-inbound' => [
                'minProcesses' => (int) env('HORIZON_WHATSAPP_INBOUND_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_WHATSAPP_INBOUND_MAX_PROCESSES', 2),
            ],
            'supervisor-whatsapp-send' => [
                'minProcesses' => (int) env('HORIZON_WHATSAPP_SEND_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_WHATSAPP_SEND_MAX_PROCESSES', 2),
            ],
            'supervisor-whatsapp-sync' => [
                'minProcesses' => (int) env('HORIZON_WHATSAPP_SYNC_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_WHATSAPP_SYNC_MAX_PROCESSES', 1),
            ],
            'supervisor-telegram-send' => [
                'minProcesses' => (int) env('HORIZON_TELEGRAM_SEND_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_TELEGRAM_SEND_MAX_PROCESSES', 2),
            ],
            'supervisor-notifications' => [
                'minProcesses' => (int) env('HORIZON_NOTIFICATIONS_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_NOTIFICATIONS_MAX_PROCESSES', 2),
            ],
            'supervisor-default' => [
                'minProcesses' => (int) env('HORIZON_DEFAULT_MIN_PROCESSES', 1),
                'maxProcesses' => (int) env('HORIZON_DEFAULT_MAX_PROCESSES', 2),
            ],
        ],

        'local' => [
            'supervisor-broadcasts' => [
                'maxProcesses' => 1,
            ],
            'supervisor-webhooks' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-download' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-variants' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-audit' => [
                'maxProcesses' => 1,
            ],
            'supervisor-face-index' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-process' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-safety' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-vlm' => [
                'maxProcesses' => 1,
            ],
            'supervisor-media-publish' => [
                'maxProcesses' => 1,
            ],
            'supervisor-whatsapp-inbound' => [
                'maxProcesses' => 1,
            ],
            'supervisor-whatsapp-send' => [
                'maxProcesses' => 1,
            ],
            'supervisor-whatsapp-sync' => [
                'maxProcesses' => 1,
            ],
            'supervisor-telegram-send' => [
                'maxProcesses' => 1,
            ],
            'supervisor-notifications' => [
                'maxProcesses' => 1,
            ],
            'supervisor-default' => [
                'maxProcesses' => 1,
            ],
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | File Watcher Configuration
    |--------------------------------------------------------------------------
    |
    | The following list of directories and files will be watched when using
    | the `horizon:listen` command. Whenever any directories or files are
    | changed, Horizon will automatically restart to apply all changes.
    |
    */

    'watch' => [
        'app',
        'bootstrap',
        'config/**/*.php',
        'database/**/*.php',
        'public/**/*.php',
        'resources/**/*.php',
        'routes',
        'composer.lock',
        'composer.json',
        '.env',
    ],
];
