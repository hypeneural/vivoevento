...
export function createEventOperationsPusher() {
  const key = getAppKey();

  if (!key) {
    console.warn('[EventOperations] Nenhuma chave WebSocket configurada para realtime.');
    return null;
  }
  ...
  if (isReverb()) {
    if (shouldDisableRealtimeInDev('EventOperations', {
      key: REVERB_KEY,
      host: REVERB_HOST,
      port: REVERB_PORT,
...
export function disconnectEventOperationsPusher() {
  if (pusherInstance) {
