...
export default function WallHubPage() {
  ...
  if (events.length === 0) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        <PageHeader
          title="Wall / Telao"
          description="Nao existe nenhum telao ativo em eventos ativos para operacao em tempo real."
        />
        <div className="rounded-3xl border border-dashed border-border bg-muted/30 px-6 py-12 text-center text-sm text-muted-foreground">
          Ative um wall a partir do detalhe do evento para ele aparecer nesta fila operacional.
        </div>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <PageHeader
        title="Wall / Telao"
        description="Lista operacional dos eventos ativos com telao em execucao ou pausado para configuracao em tempo real."
      />
            ...
            <section key={event.id} className="glass rounded-3xl border border-border/60 p-5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <div className="rounded-2xl border border-border/60 bg-background/70 px-4 py-3 text-right">
                  <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Wall code</p>
              ...
              <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {event.wall?.status === 'live'
                  ...
                  : 'Entre na configuracao para iniciar, pausar ou ajustar o slideshow.'}
              </div>
            </section>
          );
        })}
