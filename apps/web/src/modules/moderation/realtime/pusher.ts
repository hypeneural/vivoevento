...
export function createModerationPusher() {
  const key = getAppKey();

  if (!key) {
    console.warn('[Moderation] Nenhuma chave WebSocket configurada para realtime.');
    return null;
  }
  ...
  if (isReverb()) {
    if (shouldDisableRealtimeInDev('Moderation', {
      key: REVERB_KEY,
      host: REVERB_HOST,
      port: REVERB_PORT,
...
export function disconnectModerationPusher() {
  if (pusherInstance) {
