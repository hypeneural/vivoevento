...
describe('MediaAutomaticRepliesPage', () => {
  ...
  it('renders the media moderation tabs and loads the contextual configuration summary', async () => {
    ...
    expect(screen.getByRole('tab', { name: /safety objetiva/i })).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: /contexto do evento/i })).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: /laboratório/i })).toBeInTheDocument();
    ...
    expect(screen.getByLabelText(/permitir álcool/i)).not.toBeChecked();
  ...
  it('persists the objective safety configuration', async () => {
    ...
    activateTab(/safety objetiva/i);

    fireEvent.change(await screen.findByLabelText(/versao dos thresholds/i), {
      target: { value: 'global-safety-v2' },
    });
    fireEvent.click(screen.getByRole('button', { name: /salvar safety objetiva/i }));
  ...
  it('lists real event history and loads the selected production detail', async () => {
    ...
    expect(screen.getByText(/auto_publish/i)).toBeInTheDocument();
